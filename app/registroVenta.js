import { fetchDatosCliente } from "./service.js";
import {
  cargarDatos,
  guardarDatos,
  renderDetalleVenta,
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document
    ...
    .addEventListener("change", poblarDatosProducto);
  document
    .getElementById("ventaForm")
    .addEventListener("submit", handleRegistrarVenta);
  document
...
function poblarDatosProducto() {
  ...
  const producto = inventarioProductos.find(
    (p) => p.id_producto === selectedId
  );

  if (producto) {
    // Poblamos los campos dependientes
    document.getElementById("productoId").value = producto.id_producto;
    document.getElementById("productoPrecio").value = parseFloat(
      producto.precio
    ).toFixed(2);
    document.getElementById("productoCantidad").max = parseInt(
      producto.disponible
    );
  }
}
...
function agregarProducto() {
  ...
  const nombre = selectProducto.options[
    selectProducto.selectedIndex
  ].textContent
    .split("(")[0]
    .trim(); // Obtener el nombre del select
...
function eliminarProductoDetalle(id_producto) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      productosVentaActual = productosVentaActual.filter(
...
function handleRegistrarVenta(event) {
  ...
  const subtotal = productosVentaActual.reduce(
